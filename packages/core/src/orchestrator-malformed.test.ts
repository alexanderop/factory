...
// WHY: `runReview` catches per-role failures (missing findings.json, malformed
// JSON, schema mismatch) and converts them into a synthetic P3 finding via
...
// the synthetic P3 in the merged `findings.json`. Tests assert on those rather
// than on a propagated typed error.
...
describe('orchestrator — malformed harness responses', () => {
	...
	it.effect('orchestrator step with stderr-only output and exit 0 still succeeds', () =>
		Effect.gen(function* () {
	...
	it.effect('mid-stream crash via events array surfaces as HarnessExecError', () =>
		Effect.gen(function* () {
			const eventsRef = yield* Ref.make<ReadonlyArray<FactoryEvent>>([]);
			// WHY: `scriptedHarness` appends a trailing `{ type: 'exit', code: r.exitCode ?? 0 }`
			// after `events`, so the inner crash exit must also be set on `exitCode` to
			// survive that trailing event overwriting the recorded exit code.
	...
	it.scoped('review role with truncated findings.json is captured as ParseError', () =>
		Effect.gen(function* () {
			...
			// WHY: Simulates a partial-write — process killed mid-flush, file exists
			// but ends abruptly. JSON.parse fails on unterminated array.
			const exit = yield* runReviewWithRoleResponse(runDir, () => ({
				stdout: 'wrote half\n',
				writes: [
					{
